...
class PGDAttack:
    def __init__(self,
        ...
        self.noise = torch.zeros(1, 3, 256, 832)
    ...
    def evaluate(self, dataloader, results_dir, epsilon):
        ...
        for i, data in tqdm(enumerate(dataloader), total=len(dataloader)):
            tgt, ref1, ref2, self.noise = self.fgsm_untargetted(data, eps=epsilon, num_iters=num_iters, visualize=False,
                                                    save_img=self.save_adv_imgs, im_num=i+1, noise=self.noise)
    ...
    def fgsm_untargetted(self, inputs, eps, num_iters, alpha=1,
                         ...
                         im_num=None, noise=None, train_noise=True):
        ...
        #
        if train_noise:
            adv_tgt_img = adv_tgt_img + noise
            adv_tgt_img = torch.max(torch.min(adv_tgt_img, ub_depth), lb_depth)
            adv_ref_img1 = adv_ref_img1 + noise
            adv_ref_img1 = torch.max(torch.min(adv_ref_img1, ub_pose_1), lb_pose_1)
            adv_ref_img2 = adv_ref_img2 + noise
            adv_ref_img2 = torch.max(torch.min(adv_ref_img2, ub_pose_2), lb_pose_2)
        del ub_max_pose, lb_min_pose, eps_pose
        del ub_max_depth, lb_min_depth
        ...
        for i in range(num_iters):
            ...
            adv_ref_img2 = torch.max(torch.min(adv_ref_img2, ub_pose_2), lb_pose_2)
            noise = torch.max(torch.min(noise + 1/3 * (noise_pose_2 + noise_pose_1 + noise_depth), eps_depth), -eps_depth)
            # del eps_depth
        ...
        return adv_tgt_img.detach(), adv_ref_img1.detach(), adv_ref_img2.detach(), noise
...
if __name__ == '__main__':
    ...
    for epsilon in epsilons:
        attack.noise = torch.FloatTensor(attack.noise.size()).uniform_(-epsilon / 255, epsilon / 255).cuda()
        seq = 7
        ...
        noise = attack.noise * attack.ivt[1] * 255 + 50
        print(noise)
        Image.fromarray(
            np.transpose(noise.detach().cpu().squeeze().numpy(),
                         (1, 2, 0)).astype(np.uint8)
        ).save("/mnt/csip-090/SC-Sfm/mono-pose-attack-main/results/un_tar/adv_" + str(epsilon) + "_pert.png")