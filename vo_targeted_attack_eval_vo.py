...
class PGDAttack:
    def __init__(self,
        ...
        self.noise1 = torch.zeros(1, 3, 256, 832)
        self.noise2 = torch.zeros(1, 3, 256, 832)
    ...
    def evaluate(self, dataloader, results_dir, epsilon):
        ...
        for i, data in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            if i == 0:
                ...
                img1, img2, self.noise1, self.noise2 = self.targeted_attack(ref_img1, tgt_img, eps=epsilon, num_iters=num_iters, visualize=False,
                                                   save_img=self.save_adv_imgs, im_num=i, noise1=self.noise1, noise2=self.noise2)
                with torch.no_grad():
            ...
            # print(self.noise1)
            img1, img2, self.noise1, self.noise2 = self.targeted_attack(tgt_img, ref_img2, eps=epsilon, num_iters=num_iters, visualize=False,
                                              save_img=self.save_adv_imgs, im_num=i+1, noise1=self.noise1, noise2=self.noise2)
    ...
    def process_pose_inputs(self, img1, img2, target_mat):
        ...
        # compute output
        pose = self.pose_net(img1, img2) # 同时也要对pose = self.pose_net(img2, img1)处理，保证训练得到的补丁对序列有效
    ...
    def targeted_attack(self, img1, img2, eps, num_iters, alpha=1,
                         ...
                         im_num=None, noise1=None, noise2=None):
...
################################################################################################
        # 计算(img2,img1)的UAP
        if using_noise:
            adv_img1 = adv_img1 + noise2
            adv_img1 = torch.max(torch.min(adv_img1, ub_pose_1), lb_pose_1)
            adv_img2 = adv_img2 + noise1
            adv_img2 = torch.max(torch.min(adv_img2, ub_pose_2), lb_pose_2)
        ...
        loss = self.process_pose_inputs(adv_img2, adv_img1, target_mat)
        loss.backward()

        noise_img2 = 0.5 * alpha_pose * torch.sign(adv_img1.grad)
        noise_img1 = 0.5 * alpha_pose * torch.sign(adv_img2.grad)

        # print(torch.sign(adv_img1.grad) - torch.sign(adv_img2.grad))
        ...
        noise1 = torch.max(torch.min(noise1 + noise_img1, eps_pose), -eps_pose)
        noise2 = torch.max(torch.min(noise2 + noise_img2, eps_pose), -eps_pose)

        adv_img1 = adv_img1.detach()
        adv_img2 = adv_img2.detach()
###############################################################################################
        # if using_noise:
        #     adv_img1 = adv_img1 + noise_img1
        #     adv_img1 = torch.max(torch.min(adv_img1, ub_pose_1), lb_pose_1)
        #     adv_img2 = adv_img2 + noise_img2
        #     adv_img2 = torch.max(torch.min(adv_img2, ub_pose_2), lb_pose_2)
        del ub_max_pose, lb_min_pose
        ...
        for i in range(num_iters):
            ...
            noise_img1 = 0.5 * alpha_pose * torch.sign(adv_img1.grad)
            noise_img2 = 0.5 * alpha_pose * torch.sign(adv_img2.grad)
        ...
        return adv_img1.detach(), adv_img2.detach(), noise1, noise2
...
if __name__ == '__main__':
    ...
    for epsilon in epsilons:
        attack.noise1 = torch.FloatTensor(attack.noise1.size()).uniform_(-epsilon / 255, epsilon / 255).cuda()
        attack.noise2 = torch.FloatTensor(attack.noise2.size()).uniform_(-epsilon / 255, epsilon / 255).cuda()

        seq = 7
        # print("seq: " + str(seq) + " and epsilon :" + str(epsilon))
        while seq >= 0:
            print("seq: " + str(seq) + " and epsilon :" + str(epsilon))
            attack.sequence_id = '0' + str(seq)
            attack.eval_split = '0' + str(seq)
            normalize = custom_transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                    std=[0.229, 0.224, 0.225])
            test_transform = custom_transforms.Compose([custom_transforms.ArrayToTensor(), normalize])
            attack.test_set = SequenceFolder(
                root=attack.data_path,
                transform=test_transform,
                seq=attack.sequence_id,
                sequence_length=3,
                dataset='kitti'
            )
        ...
        noise1 = attack.noise1 * attack.ivt[1] * 255 + 50
        noise2 = attack.noise2 * attack.ivt[1] * 255 + 50

        print(noise1)
        Image.fromarray(
            np.transpose(noise1.detach().cpu().squeeze().numpy(),
                         (1, 2, 0)).astype(np.uint8)
        ).save("/mnt/csip-090/SC-Sfm/mono-pose-attack-main/results/flip_yaw/adv_noise1_" + str(epsilon) + "_pert.png")
        Image.fromarray(
            np.transpose(noise1.detach().cpu().squeeze().numpy(),
                         (1, 2, 0)).astype(np.uint8)
        ).save("/mnt/csip-090/SC-Sfm/mono-pose-attack-main/results/flip_yaw/adv_noise2_" + str(epsilon) + "_pert.png")
